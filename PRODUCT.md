...
- PWA-onderdelen: `src/data/pwa.ts` (naam, kleuren, precache-lijst), `src/pages/manifest.webmanifest.ts` (gegenereerd, volgt de base path), `src/sw/service-worker.js` (sjabloon) via `src/pages/sw.js.ts`, `src/sw/registratie.js` (registratie + opruimen van vreemde workers, inline in de head), `/offline/` als vangnetpagina en `scripts/genereer-iconen.mjs` voor de iconen in `public/icons/`.
