 ...
 * `src/content/archief/`.
 *
 * Draaien:  node scripts/archive-convert.mjs [--dry]
...
const jcColumns = columns(sql, 's8hxk_jcomments');
const commentsByArticle = new Map();
for (const r of readTable(sql, 's8hxk_jcomments')) {
  const o = Object.fromEntries(jcColumns.map((c, i) => [c, r[i]]));
  if (o.published !== '1' || o.object_group !== 'com_content') continue;
  const list = commentsByArticle.get(Number(o.object_id)) || [];
  list.push({
  ...
  commentsByArticle.set(Number(o.object_id), list);
}
...
const stats = {
  imagesKept: 0, imagesDropped: 0, smileys: 0, textSmileys: 0,
...
function cleanHtml(html, article) {
  ...
  h = h.replace(/<img([^>]*)>/gi, (tag, attrs) => {
    ...
    if (!hit || hit.dood || !hit.bestand) {
      stats.imagesDropped++;
      return '';
    }
    stats.imagesKept++;
    const alt = (attrs.match(/alt\s*=\s*"([^"]*)"/i) || [])[1] || '';
    return `<img src="/archief/beelden/${hit.bestand}" alt="${alt.replace(/"/g, '')}">`;
  });
  ...
  h = h.replace(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi, (m, attrs, content) => {
    const href = ent((attrs.match(/href\s*=\s*"([^"]*)"/i) || attrs.match(/href\s*=\s*'([^']*)'/i) || [])[1] || '').trim();
    if (!href || href.startsWith('#')) { stats.linksUnlinked++; return content; }
    ...
    if (hit && !hit.dood && hit.bestand) {
      stats.linksKept++;
      return `<a href="/archief/beelden/${hit.bestand}">${content}</a>`;
    }
    ...
    if (dood) { stats.linksUnlinked++; return content; }
    stats.linksKept++;
...
for (const a of unique.slice().sort((x, y) => dateOf(x).localeCompare(dateOf(y)) || x.id - y.id)) {
  ...
  const comments = (commentsByArticle.get(a.id) || [])
  ...
  const group = GROUP_BY_CATID[a.catid] ?? null;
...
if (!DRY) {
  ...
  for (const b of files) {
    fs.writeFileSync(path.join(TARGET, b.name), b.content);
    written++;
  }
}
...
console.log(`Met reacties:    ${withComments} artikels, ${[...commentsByArticle.values()].flat().length} reacties totaal`);
console.log(`Leeg, dus weg:   ${empty}`);
for (const r of skippedEmpty) console.log(`                 ${r}`);
console.log('');
console.log('Inhoud:');
console.log(`  beelden behouden   ${stats.imagesKept}`);
console.log(`  beelden verwijderd ${stats.imagesDropped}  (bron dood, zie manifest)`);
...
const authorCounts = {};
for (const b of files) { const n = authorOf(b.a) || '—'; authorCounts[n] = (authorCounts[n] || 0) + 1; }
console.log('\nAuteurs na normalisatie:');
Object.entries(authorCounts).sort((a, b) => b[1] - a[1]).forEach(([k, v]) => console.log(`  ${String(v).padStart(4)} ${k}`));

const groupCounts = {};
for (const b of files) { const g = GROUP_BY_CATID[b.a.catid] ?? 'geen groep'; groupCounts[g] = (groupCounts[g] || 0) + 1; }
