...
//   newStanding()   → SeasonManager::calculateCurrentSeason()
...
export const fullName = (player: Player) => `${player.voornaam} ${player.naam}`;
...
export function draw(byStrength: Player[], random: () => number = Math.random): DrawnCourt[] {
  ...
  function pickFour(pool: Player[], source: CourtSource) {
    const picked: Player[] = [];
    for (let i = 0; i < 4; i++) {
      picked.push(...pool.splice(Math.floor(random() * pool.length), 1));
    }
    courts.push({ source, spelers: picked });
    return picked;
  }

  function removeFrom(picked: Player[], pool: Player[]) {
    for (const player of picked) {
      const i = pool.indexOf(player);
      if (i > -1) pool.splice(i, 1);
    }
  }

  while (upper.length >= 4 || lower.length >= 4) {
    if (upper.length >= 4) removeFrom(pickFour(upper, 'boven'), lower);
    if (lower.length >= 4) removeFrom(pickFour(lower, 'onder'), upper);
  }

  const rest = [...upper, ...lower].filter(
    (player, i, all) => all.findIndex((other) => other.id === player.id) === i,
  );
  if (rest.length >= 4) pickFour(rest, 'rest');
  if (rest.length > 0) courts.push({ source: 'uitgeloot', spelers: [...rest] });
...
export function isHome(position: number, set: number): boolean {
...
export function setPoints(sets: readonly [number, number][], position: number): number[] {
  return SET_LINEUP.map((_, set) => {
    ...
    return isHome(position, set) ? trimSet(home, away) : trimSet(away, home);
  });
}
...
export function headStart(bonuses: readonly number[], set: number) {
  const { home, away } = SET_LINEUP[set];
  const sumOf = (positions: readonly number[]) =>
    positions.reduce<number>((sum, i) => sum + bonuses[i], 0);
 ...
 * seizoen vordert.
 */
export function newStanding(previousAverage: number, dayScore: number, speeldagen: number): number {
  return (previousAverage * speeldagen + dayScore) / (speeldagen + 1);
}

/** Vaste komma en Belgische komma, zoals de rest van de site cijfers toont. */
export function decimalComma(value: number, decimals = 2): string {
