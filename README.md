...
| Onderwerpen | `intraclub` (automatisch zodra een nieuwe speeldag berekend is, met `url` naar `/intraclub/speeldag/?id=…`) en `club` (met de hand, occasioneel: iets extra naast Twizzit, zoals een open speeldag of een lessenreeks). De lijst staat in `PUSH_TOPICS`; een nieuw onderwerp moet aan beide kanten |
